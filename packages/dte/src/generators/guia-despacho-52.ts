...
export function generateGuiaDespacho52(data: DocumentData): string {
  ...
  if (data.transport) {
    doc.ele('Transporte')
      .ele('Patente').txt(data.transport.patente).up()
      .ele('RUTTrans').txt(data.transport.rutTransporter).up()
      .ele('DirDest').txt(data.receiver.address).up()
      .ele('CmnaDest').txt(data.receiver.commune).up()
      .ele('CiudadDest').txt(data.receiver.city).up()
      .up()
  }
