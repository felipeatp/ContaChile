        ...
        <div className="p-4 space-y-3">
          <div className="rounded-md bg-muted p-3 text-sm">
            ...
            <div className="text-xs">
              {new Date(movement.postedAt).toLocaleDateString('es-CL')} • {movement.type === 'CREDIT' ? '+' : '-'}{format(movement.amount)} • {movement.counterpartName || '—'}
            </div>
            {sug && (
              <div className="mt-2 text-xs text-blue-700">
                💡 Sugerencia IA: {sug.clasificacion} ({sug.codigo_cuenta}) — confianza {Math.round(sug.confianza * 100)}%
              </div>
            )}
          </div>
