...
export default function MovimientosPage() {
  ...
  const { data: products = [], isLoading: productsLoading, isError: productsError } =
    useInventoryProducts({ active: true })
  ...
  } = useInventoryMovements(selectedId)
    ...
    <div className="space-y-8 animate-fade-up">
      ...
      <section className="card-editorial p-5">
        ...
        <QueryState
          ...
          errorMessage="No pudimos cargar la lista de productos."
        >
          <select
            value={selectedId}
      ...
      {!selectedId ? (
        ...
        <QueryState
          isLoading={kardexLoading}
          isError={kardexError}
          onRetry={() => refetchKardex()}
          errorMessage="No pudimos cargar los movimientos."
        >
          {kardex && (
            <>
              <section>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-xs text-muted-foreground/60 font-mono">
                    {kardex.product.code} · {kardex.product.name}
                  </span>
                </div>
                <div className="grid gap-4 md:grid-cols-3">
                  <Stat label="Stock actual" value={`${kardex.product.stock} ${kardex.product.unit}`} tone="default" />
                  <Stat label="Costo promedio" value={formatCLP(kardex.product.costPrice)} tone="default" />
                  <Stat
                    label="Valor inventario"
                    value={formatCLP(kardex.product.stock * kardex.product.costPrice)}
                    tone="accent"
                  />
                </div>
              </section>
              ...
              <section>
                <div className="flex items-end justify-between mb-4">
                  ...
                  <span className="text-xs text-muted-foreground/60 font-mono">
                    {kardex.movements.length} líneas
                  </span>
                </div>
                ...
                <div className="card-editorial overflow-hidden">
                  <QueryState
                    isLoading={false}
                    isError={false}
                    isEmpty={kardex.movements.length === 0}
    ...
    <Modal
      ...
      footer={
        <div className="flex justify-end gap-2">
          <Button variant="outline" onClick={onClose} disabled={createMovement.isPending}>
            Cancelar
          </Button>
          <Button onClick={submit} disabled={createMovement.isPending || quantity === 0}>
