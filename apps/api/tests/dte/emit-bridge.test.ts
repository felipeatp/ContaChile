...
describe('POST /dte/emit-bridge', () => {
  ...
  it('returns existing document for duplicate idempotency key', async () => {
    ...
    const response = await app.inject({
      method: 'POST',
      url: '/dte/emit-bridge',
      headers: { 'x-company-id': 'company-123', 'Idempotency-Key': 'bridge-key-1' },
      payload: {
        type: 33,
        receiver: {
          rut: '12345678-5',
    ...
    expect(response.statusCode).toBe(201)
    const body = JSON.parse(response.body)
    expect(body.id).toBe('doc-bridge-existing')
    expect(body.trackId).toBe('ACEPTA-OLD')
    expect(mockPrisma.document.create).not.toHaveBeenCalled()
    expect(mockPrisma.document.findUnique).toHaveBeenCalledWith({
      where: { idempotencyKey: 'bridge-key-1' },
    })
  })
})